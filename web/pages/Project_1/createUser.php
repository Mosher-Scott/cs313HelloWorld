<?php
  ...
  if(isset($_POST) && isset($_POST['action']) == 'editUser') {
    ...
    if(empty($firstName) || empty($lastName) || empty($email) || empty($address) || empty($city) || empty($state) || empty($zipcode) || empty($phone) || empty($displayName) || empty($password)) {
      ...
      $userInfo[0]['user_role'] = $user_role;
      ...
      $recordsUpdated = registerNewUser($id, $firstName, $lastName, $email, $phone, $address, $city, $state, $zipcode, $displayName, $password, $user_role);
